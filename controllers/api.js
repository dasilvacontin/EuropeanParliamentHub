...
exports.getPartyDB = function(req, res) {
  res.render('api/party', {
    title: 'Party DB API'
  });
};
...
exports.postPartyDB = function(req, res) {
  ...
  party.save(function(err) {
    if (err) {
      ...
      return res.redirect('/api/party');
    }
    req.flash('success', { msg: req.body.name +' was saved successfully in database!' });
    return res.redirect('/api/party');
  });
...
exports.getPersonDB = function(req, res) {
  res.render('api/person', {
    title: 'Person DB API'
  });
};
...
exports.postPersonDB = function(req, res) {
  console.log(req.body);
  ...
  var person = new Person({
    ...
    deputyForParty: req.body.deputyForParty,
    candidateForParty: req.body.candidateForParty,
  ...
  person.save(function(err) {
    if (err) {
      if (err.code === 11000) {
        req.flash('errors', { msg: 'Representative with that name already exists.' });
      }
